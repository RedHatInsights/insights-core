...
def get_cmdline(context):
    ...
    If more than one properties have the same key, the corresponding values are stored in a list.
    ...
    """
    cmdline_properties = {}
    line = context.content[0]
    for property in line.strip().split():
        if not property:
            continue
        elif "=" not in property:
            cmdline_properties[property] = None
        else:
