...
@mapper("ovs-vsctl_show")
class OVSvsctlshow(MapperOutput):
    @staticmethod
    def parse_content(content):
       ...
       ===============================
        ...
        result_dict = {"uuid": content[0], "bridges": {}, "ovs_version": version_line.split('"')[1]}
        bridge_dict = {}
        port_dict = {}
        for line in content[1:-1]:
            ...
            if prefix == "Bridge":
                bridge_dict = {"ports": []}
                result_dict["bridges"][value] = bridge_dict
                ...
                port_dict["options"] = {item.split("=")[0]: item.split("=")[1].replace('"', "") for item in
                                        options_items}
        return result_dict

    def get_ovs_version(self):
        return self.get("ovs_version")

    def get_bridge(self, bridge_name):
        return self.get("bridges").get(bridge_name, None)
