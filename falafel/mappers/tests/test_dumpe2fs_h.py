...
class TestDumpe2fs():
    def test_dumpe2fs(self):
        ...
        assert "/dev/mapper/vg_spcdrhellb01-lv_usr" in dumpe2fs_dict
        assert dumpe2fs_dict["/dev/mapper/vg_spcdrhellb01-lv_usr"].get("Filesystem features") == [
            'has_journal',
            'ext_attr',
            'resize_inode',
            'dir_index',
        ...
        assert dumpe2fs_dict["/dev/mapper/vg_spcdrhellb01-lv_usr"].get("Default mount options") == [
            'user_xattr',
            'acl',
            'journal_data_writeback'
        ]
        assert dumpe2fs_dict == {
            '/dev/mapper/vg_spcdrhellb01-lv_usr': {
                'Filesystem magic number': '0xEF53',
                'Filesystem flags': 'signed_directory_hash',
                'Filesystem revision #': '1 (dynamic)',
                'Default mount options': ['user_xattr', 'acl', 'journal_data_writeback'],
