...
class TestMount():
    def test_mount(self):
        ...
        assert mount_list == [{
            'mount_type': 'tmpfs',
            'mount_point': '/tmp',
            'mount_clause': 'tmpfs on /tmp type tmpfs (rw,seclabel)',
            'mount_options': ['rw', 'seclabel'],
            'filesystem': 'tmpfs'
        }, {
            'mount_type': 'hugetlbfs',
            'mount_point': '/dev/hugepages',
            'mount_clause': 'hugetlbfs on /dev/hugepages type hugetlbfs (rw,relatime,seclabel)',
            'mount_options': ['rw', 'relatime', 'seclabel'],
            'filesystem': 'hugetlbfs'
        }, {
            'mount_type': 'nfsd',
            'mount_point': '/proc/fs/nfsd',
            'mount_clause': 'nfsd on /proc/fs/nfsd type nfsd (rw,relatime)',
            'mount_options': ['rw', 'relatime'],
            'filesystem': 'nfsd'
        }, {
            'mount_type': 'ext4',
            'mount_point': '/boot',
            'mount_clause': '/dev/sda1 on /boot type ext4 (rw,relatime,seclabel,data=ordered)',
            'mount_options': ['rw', 'relatime', 'seclabel', 'data=ordered'],
            'filesystem': '/dev/sda1'
        }, {
            'mount_type': 'ext4',
            'mount_point': '/home',
            'mount_clause': '/dev/mapper/fedora-home on /home type ext4 (rw,relatime,seclabel,data=ordered)',
            'mount_options': ['rw', 'relatime', 'seclabel', 'data=ordered'],
            'filesystem': '/dev/mapper/fedora-home'
        }, {
            'mount_type': 'rpc_pipefs',
            'mount_point': '/var/lib/nfs/rpc_pipefs',
            'mount_clause': 'sunrpc on /var/lib/nfs/rpc_pipefs type rpc_pipefs (rw,relatime)',
            'mount_options': ['rw', 'relatime'],
            'filesystem': 'sunrpc'
        }, {
            'mount_type': 'tmpfs',
            'mount_point': '/run/user/42',
            'mount_clause': 'tmpfs on /run/user/42 type tmpfs (rw,nosuid,nodev,relatime,seclabel,size=1605428k,mode=700,uid=42,gid=42)',
            'mount_options': [
                'rw',
                'nosuid',
                'nodev',
            ...
            'mount_clause': 'tmpfs on /run/user/1000 type tmpfs (rw,nosuid,nodev,relatime,seclabel,size=1605428k,mode=700,uid=1000,gid=1000)',
            'mount_options': [
                'rw',
                'nosuid',
                'nodev',
            ...
            'mount_type': 'fuse.gvfsd-fuse',
            'mount_point': '/run/user/1000/gvfs',
            'mount_clause': 'gvfsd-fuse on /run/user/1000/gvfs type fuse.gvfsd-fuse (rw,nosuid,nodev,relatime,user_id=1000,group_id=1000)',
            'mount_options': ['rw', 'nosuid', 'nodev', 'relatime', 'user_id=1000', 'group_id=1000'],
            'filesystem': 'gvfsd-fuse'
        }, {
            'mount_type': 'fusectl',
            'mount_point': '/sys/fs/fuse/connections',
            'mount_clause': 'fusectl on /sys/fs/fuse/connections type fusectl (rw,relatime)',
            'mount_options': ['rw', 'relatime'],
            'filesystem': 'fusectl'
        }, {
            'mount_type': 'ext4',
            'mount_point': '/etc/shadow',
            'mount_clause': '/dev/mapper/HostVG-Config on /etc/shadow type ext4 (rw,noatime,seclabel,stripe=256,data=ordered) [CONFIG]',
            'mount_options': [
                'rw',
                'noatime',
                'seclabel',
