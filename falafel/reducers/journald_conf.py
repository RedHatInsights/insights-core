...
Shared reducer for parsing of journald configuration. man journald.conf describes where various
journald config files can reside and how they take precedence one over another. The reducer
...
The mappers this reducer uses process only active settings (lines that are not commented out). The
...
from falafel.mappers.journald_conf import EtcJournaldConf, EtcJournaldConfD, UsrJournaldConfD


# TODO - further falafel work - convert this to a generic option & file priority evaluator for
#        other reducers

@reducer(requires=[EtcJournaldConf], optional=[EtcJournaldConfD, UsrJournaldConfD], shared=True)
class JournaldConfAll(object):
    """
    Reducer for accessing files from the mappers EtcJournaldConf, EtcJournaldConfD, UsrJournaldConfD
    ...
    Can be later refactored to a reducer for parsing all configuration files with key=option lines,
    like journald files.
    ...
    * A single option value is overwritten when another file with a higher priority has an option
    ...
    def __init__(self, local, shared):
        ...
        # preparation for future possible refactoring into a more general reducer
        central_file_lowest_prio = True

        # comments in this method describe journald configuration; it should work for similar ones
        etc_confd = {}  # mapper instances indexed by file name
        usr_confd = {}  # mapper instances indexed by file name
        if EtcJournaldConfD in shared:
            for mapper_instance in shared[EtcJournaldConfD]:
                etc_confd[mapper_instance.file_name] = mapper_instance
        if UsrJournaldConfD in shared:
            for mapper_instance in shared[UsrJournaldConfD]:
                usr_confd[mapper_instance.file_name] = mapper_instance
        ...
        for file_name, mapper_instance in usr_confd.iteritems():
            effective_confd[file_name] = mapper_instance
        ...
        #  counterparts:
        for file_name, mapper_instance in etc_confd.iteritems():
        ...
        central_mapper = shared[EtcJournaldConf][0]
        mappers_list = [effective_confd[file_name] for file_name in sorted_file_names]
        if central_file_lowest_prio:
            mappers_list = [central_mapper] + mappers_list
        else:
            mappers_list = mappers_list + [central_mapper]
        ...
        for mapper_instance in mappers_list:
            if mapper_instance.active_settings:  # do not iterate if empty or None
                if mapper_instance.file_path:
                    # empty and None file names are not added (that is invalid anyway), see test_11
                    files_used_priority_order.append(mapper_instance.file_path)
                for k, v in mapper_instance.active_settings.iteritems():
                    resulting_options_with_file_name[k] = (v, mapper_instance.file_path)

        # not named simply as `active_settings` so as not to confuse the contents with the
        #  `active_settings` dictionary in mappers/journald_conf.py
