...
ceph_config_show = sf.foreach_execute(ceph_socket_files, "/usr/bin/ceph daemon %s config show", name="ceph_config_show")
...
ceph_osd_ec_profile_get = sf.foreach_execute(ceph_osd_ec_profile_ls, "/usr/bin/ceph osd erasure-code-profile get %s -f json-pretty", name="ceph_osd_ec_profile_get")
...
docker_image_inspect = sf.foreach_execute(docker_image_ids, "/usr/bin/docker inspect %s", name="docker_image_inspect", context=DockerHostContext)
docker_container_inspect = sf.foreach_execute(docker_container_ids, "/usr/bin/docker inspect %s", name="docker_container_inspect", context=DockerHostContext)
...
dumpe2fs_h = sf.foreach_execute(dumpdev, "/sbin/dumpe2fs -h %s", name="dumpe2fs_h", alias="dumpe2fs-h")
...
dcbtool_gc_dcb = sf.foreach_execute(ethernet_interfaces, "/sbin/dcbtool gc %s dcb", name="dcbtool_gc_dcb")
ethtool = sf.foreach_execute(ethernet_interfaces, "/sbin/ethtool %s", name="ethtool")
ethtool_S = sf.foreach_execute(ethernet_interfaces, "/sbin/ethtool -S %s", name="ethtool_S", alias="ethtool-S")
ethtool_a = sf.foreach_execute(ethernet_interfaces, "/sbin/ethtool -a %s", name="ethtool_a", alias="ethtool-a")
ethtool_c = sf.foreach_execute(ethernet_interfaces, "/sbin/ethtool -c %s", name="ethtool_c", alias="ethtool-c")
ethtool_g = sf.foreach_execute(ethernet_interfaces, "/sbin/ethtool -g %s", name="ethtool_g", alias="ethtool-g")
ethtool_i = sf.foreach_execute(ethernet_interfaces, "/sbin/ethtool -i %s", name="ethtool_i", alias="ethtool-i")
ethtool_k = sf.foreach_execute(ethernet_interfaces, "/sbin/ethtool -k %s", name="ethtool_k", alias="ethtool-k")
...
ip_s_link = sf.simple_command("/sbin/ip -s link", name="ip_s_link", alias="ip-s_link")
...
ipcs_s_i = sf.foreach_execute(semid, "/usr/bin/ipcs -s -i %s", name="ipcs_s_i")
...
modinfo = sf.foreach_execute(module, "/usr/sbin/modinfo %s", name="modinfo")
...
netstat_agn = sf.simple_command("/bin/netstat -agn", name="netstat__agn", alias="netstat_-agn")
netstat_i = sf.simple_command("/bin/netstat -i", name="netstat_i", alias="netstat-i")
netstat_s = sf.simple_command("/bin/netstat -s", name="netstat_s", alias="netstat-s")
...
oc_get_bc = sf.simple_command("/usr/bin/oc get bc -o yaml --all-namespaces", name="oc_get_bc", context=OpenShiftContext)
...
openshift_certificates = sf.foreach_execute(crt, "/usr/bin/openssl x509 -noout -enddate -in %s", name="openshift_certificates")
...
package_provides_java = sf.foreach_execute(running_java, "echo %s $(readlink -e `which %s` | xargs rpm -qf)", name="package_provides_java")
...
proc_snmp_ipv4 = sf.simple_file("proc/net/snmp", name="proc_snmp_ipv4")
proc_snmp_ipv6 = sf.simple_file("proc/net/snmp6", name="proc_snmp_ipv6")
...
scheduler = sf.foreach_collect(block_devices, "/sys/block/%s/queue/scheduler", name="scheduler")
...
smartctl = sf.foreach_execute(block, "/sbin/smartctl -a %s", name="smartctl", keep_rc=True)
...
swift_object_expirer_conf = sf.simple_file("etc/swift/object-expirer.conf", name="swift_object_expirer_conf")
swift_proxy_server_conf = sf.simple_file("etc/swift/proxy-server.conf", name="swift_proxy_server_conf")
...
systemid = sf.first_of([sf.simple_file("/etc/sysconfig/rhn/systemid", name="system_id_etc"),
                        ...
                        name="systemid")
teamdctl_state_dump = sf.foreach_execute(ethernet_interfaces, "/usr/bin/teamdctl %s state dump", name="teamdctl_state_dump")
