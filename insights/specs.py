...
catalina_out = sf.first_of([sf.foreach(tomcat_base, "%s/logs/catalina.out"),
                            sf.glob_file("tomcat-logs/tomcat*/catalina.out", context=HostArchiveContext)],
                           name="catalina_out", alias="catalina.out")
catalina_server_log = sf.first_of([sf.foreach(tomcat_base, "%s/logs/catalina*.log"),
...
tomcat_vdc_targeted = sf.foreach(tomcat_home_base, "/bin/grep -R -s 'VirtualDirContext' --include '*.xml' %s", name="tomcat_vdc_targeted")
