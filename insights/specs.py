...
blkid = sf.simple_command("/usr/sbin/blkid -c /dev/null", name="blkid", alias="blkid")
bond = sf.glob_file("/proc/net/bonding/bond*", name="bond", alias="bond")
branch_info = sf.simple_file("/branch_info", name="branch_info", alias="branch_info")
brctl_show = sf.simple_command("/usr/sbin/brctl show", name="brctl_show", alias="brctl_show")
...
cciss = sf.glob_file("/proc/driver/cciss/cciss*", name="cciss", alias="cciss")
ceilometer_central_log = sf.simple_file("/var/log/ceilometer/central.log", name="ceilometer_central_log", alias="ceilometer_central_log")
ceilometer_collector_log = sf.simple_file("/var/log/ceilometer/collector.log", name="ceilometer_collector_log", alias="ceilometer_collector_log")
...
ceph_s = sf.simple_command("/usr/bin/ceph -s -f json-pretty", name="ceph_s", alias="ceph_s")
ceph_v = sf.simple_command("/usr/bin/ceph -v", name="ceph_v", alias="ceph_v")
chkconfig = sf.simple_command("/sbin/chkconfig --list", name="chkconfig", alias="chkconfig")
...
cmdline = sf.simple_file("/proc/cmdline", name="cmdline", alias="cmdline")
cobbler_settings = sf.first_file(["/etc/cobbler/settings", "/conf/cobbler/settings"], name="cobbler_settings", alias="cobbler_settings")
cobbler_modules_conf = sf.first_file(["/etc/cobbler/modules.conf", "/conf/cobbler/modules.conf"], name="cobbler_modules_conf")
corosync = sf.simple_file("/etc/sysconfig/corosync", name="corosync", alias="corosync")
cpuinfo = sf.first_file(["/proc/cpuinfo", "/cpuinfo"], name="cpuinfo", alias="cpuinfo")
...
date = sf.simple_command("/bin/date", name="date", alias="date")
date_iso = sf.simple_command("/bin/date --iso-8601=seconds", name="date_iso", alias="date_iso")
date_utc = sf.simple_command("/bin/date --utc", name="date_utc", alias="date_utc")
df_al = sf.simple_command("/bin/df -al", name="df_al", alias="df_al")
df_alP = sf.simple_command("/bin/df -alP", name="df_alP", alias="df_alP")
df_li = sf.simple_command("/bin/df -li", name="df_li", alias="df_li")
dirsrv = sf.simple_file("/etc/sysconfig/dirsrv", name="dirsrv", alias="dirsrv")
display_java = sf.simple_command("/usr/sbin/alternatives --display java", name="display_java", alias="display_java")
...
docker_info = sf.simple_command("/usr/bin/docker info", name="docker_info", alias="docker_info")
docker_list_containers = sf.simple_command("/usr/bin/docker ps --all --no-trunc", name="docker_list_containers", alias="docker_list_containers")
docker_list_images = sf.simple_command("/usr/bin/docker images --all --no-trunc --digests", name="docker_list_images", alias="docker_list_images")
...
ethtool = sf.foreach(ethernet_interfaces, "/sbin/ethtool %s", name="ethtool", alias="ethtool")
ethtool_S = sf.foreach(ethernet_interfaces, "/sbin/ethtool -S %s", name="ethtool_S", alias="ethtool_S")
ethtool_a = sf.foreach(ethernet_interfaces, "/sbin/ethtool -a %s", name="ethtool_a", alias="ethtool_a")
ethtool_c = sf.foreach(ethernet_interfaces, "/sbin/ethtool -c %s", name="ethtool_c", alias="ethtool_c")
ethtool_g = sf.foreach(ethernet_interfaces, "/sbin/ethtool -g %s", name="ethtool_g", alias="ethtool_g")
ethtool_i = sf.foreach(ethernet_interfaces, "/sbin/ethtool -i %s", name="ethtool_i", alias="ethtool_i")
ethtool_k = sf.foreach(ethernet_interfaces, "/sbin/ethtool -k %s", name="ethtool_k", alias="ethtool_k")
facter = sf.simple_command("/usr/bin/facter", name="facter", alias="facter")
fc_match = sf.simple_command("/usr/bin/fc-match -sv 'sans:regular:roman' family fontformat", name="fc_match", alias="fc_match")
fdisk_l = sf.simple_command("/sbin/fdisk -l", name="fdisk_l", alias="fdisk_l")
...
fstab = sf.simple_file("/etc/fstab", name="fstab", alias="fstab")
galera_cnf = sf.simple_file("/etc/my.cnf.d/galera.cnf", name="galera_cnf", alias="galera.cnf")
getenforce = sf.simple_command("/usr/sbin/getenforce", name="getenforce", alias="getenforce")
getsebool = sf.simple_command("/usr/sbin/getsebool -a", name="getsebool", alias="getsebool")
glance_api_conf = sf.simple_file("/etc/glance/glance-api.conf", name="glance_api_conf", alias="glance-api.conf")
glance_api_log = sf.simple_file("/var/log/glance/api.log", name="glance_api_log", alias="glance_api_log")
...
grub2_cfg = sf.simple_file("/boot/grub2/grub.cfg", name="grub2_cfg", alias="grub2.conf")
...
hammer_ping = sf.simple_command("/usr/bin/hammer ping", name="hammer_ping", alias="hammer_ping")
haproxy_cfg = sf.simple_file("/etc/haproxy/haproxy.cfg", name="haproxy_cfg", alias="haproxy_cfg")
...
heat_crontab = sf.simple_command("/usr/bin/crontab -l -u heat", name="heat_crontab", alias="heat_crontab")
...
hponcfg_g = sf.simple_command("/sbin/hponcfg -g", name="hponcfg_g", alias="hponcfg_g")
httpd_access_log = sf.simple_file("/var/log/httpd/access_log", name="httpd_access_log", alias="httpd_access_log")
httpd_conf = sf.first_file(["/etc/httpd/conf/httpd.conf", "/conf/httpd/conf/httpd.conf"], name="httpd_conf", alias="httpd.conf")
...
httpd_V = sf.simple_command("/usr/sbin/httpd -V", name="http_V", alias="http_V")
ifcfg = sf.glob_file("/etc/sysconfig/network-scripts/ifcfg-*", name="ifcfg", alias="ifcfg")
ifconfig = sf.simple_command("/sbin/ifconfig -a", name="ifconfig", alias="ifconfig")
...
interrupts = sf.simple_file("/proc/interrupts", name="interrupts", alias="interrupts")
ip_addr = sf.simple_command("/sbin/ip addr", name="ip_addr", alias="ip_addr")
ip_route_show_table_all = sf.simple_command("/sbin/ip route show table all", name="ip_route_show_table_all", alias="ip_route_show_table_all")
ipaupgrade_log = sf.simple_file("/var/log/ipaupgrade.log", name="ipaupgrade_log", alias="ipaupgrade_log")
ipcs_s = sf.simple_command("/usr/bin/ipcs -s", name="ipcs_s", alias="ipcs_s")
semid = sf.simple_command("/usr/bin/ipcs -s | awk '{if (NF == 5 && $NF ~ /^[0-9]+$/) print $NF}'", name="semid", alias="semid")
ipcs_s_i = sf.foreach(semid, "/usr/bin/ipcs -s -i %s", name="ipcs_s_i", alias="ipcs_s_i")
iptables = sf.simple_command("/sbin/iptables-save", name="iptables", alias="iptables")
iptables_permanent = sf.simple_file("etc/sysconfig/iptables", name="iptables_permanent", alias="iptables_permanent")
ip6tables = sf.simple_command("/sbin/ip6tables-save", name="ip6tables", alias="ip6tables")
ip6tables_permanent = sf.simple_file("etc/sysconfig/ip6tables", name="ip6tables_permanent", alias="ip6tables_permanent")
ipv4_neigh = sf.simple_command("/sbin/ip -4 neighbor show nud all", name="ipv4_neigh", alias="ipv4_neigh")
ipv6_neigh = sf.simple_command("/sbin/ip -6 neighbor show nud all", name="ipv6_neigh", alias="ipv6_neigh")
kdump = sf.simple_file("/etc/sysconfig/kdump", name="kdump", alias="kdump")
...
locale = sf.simple_command("/usr/bin/locale", name="locale", alias="locale")
localtime = sf.simple_command("/usr/bin/file -L /etc/localtime", name="localtime", alias="localtime")
lpstat_p = sf.simple_command("/usr/bin/lpstat -p", name="lpstat_p", alias="lpstat_p")
lsblk = sf.simple_command("/bin/lsblk", name="lsblk", alias="lsblk")
...
lscpu = sf.simple_command("/usr/bin/lscpu", name="lscpu", alias="lscpu")
...
lsmod = sf.simple_command("/sbin/lsmod", name="lsmod", alias="lsmod")
lspci = sf.simple_command("/sbin/lspci", name="lspci", alias="lspci")
lsof = sf.simple_command("/usr/sbin/lsof", name="lsof", alias="lsof")
lssap = sf.simple_command("/usr/sap/hostctrl/exe/lssap", name="lssap", alias="lssap")
ls_boot = sf.simple_command("/bin/ls -lanR /boot", name="ls_boot", alias="ls_boot")
ls_docker_volumes = sf.simple_command("/bin/ls -lanR /var/lib/docker/volumes", name="ls_docker_volumes")
ls_dev = sf.simple_command("/bin/ls -lanR /dev", name="ls_dev", alias="ls_dev")
ls_disk = sf.simple_command("/bin/ls -lanR /dev/disk/by-*", name="ls_disk", alias="ls_disk")
...
lvdisplay = sf.simple_command("/sbin/lvdisplay", name="lvdisplay", alias="lvdisplay")
...
mdstat = sf.simple_file("/proc/mdstat", name="mdstat", alias="mdstat")
meminfo = sf.first_file(["/proc/meminfo", "/meminfo"], name="meminfo", alias="memoinfo")
messages = sf.simple_file("/var/log/messages", name="messages", alias="messages")
...
netconsole = sf.simple_file("/etc/sysconfig/netconsole", name="netconsole", alias="netconsole")
netstat = sf.simple_command("/bin/netstat -neopa", name="netstat", alias="netstat")
netstat_agn = sf.simple_command("/bin/netstat -agn", name="netstat_agn", alias="netstat_agn")
netstat_i = sf.simple_command("/bin/netstat -i", name="netstat_i", alias="netstat_i")
netstat_s = sf.simple_command("/bin/netstat -s", name="netstat_s", alias="netstat_s")
...
nfs_exports = sf.simple_file("/etc/exports", name="nfs_exports", alias="nfs_exports")
nfs_exports_d = sf.glob_file("/etc/exports.d/*.exports", name="nfs_exports_d")
nova_api_log = sf.simple_file("/var/log/nova/nova-api.log", name="nova_api_log", alias="nova_api.log")
nova_compute_log = sf.simple_file("/var/log/nova/nova-compute.log", name="nova_compute_log", alias="nova_compute.log")
nova_conf = sf.simple_file("/etc/nova/nova.conf", name="nova_conf", alias="nova.conf")
nproc_conf = sf.glob_file("/etc/security/limits.d/*-nproc.conf", name="nproc_conf", alias="nproc.conf")
...
ps_aux = sf.simple_command("/bin/ps aux", name="ps_aux", alias="ps_aux")
ps_auxcww = sf.simple_command("/bin/ps auxcww", name="ps_auxcww", alias="ps_auxcww")
ps_auxwww = sf.simple_file("/sos_commands/process/ps_auxwww", name="ps_auxwww", alias="ps_auxwww")
ps_axcwwo = sf.simple_command("/bin/ps axcwwo ucomm,%cpu,lstart", name="ps_axcwwo", alias="ps_axcwwo")
puppet_ssl_cert_ca_pem = None
pvs = sf.simple_command('/sbin/pvs -a -v -o +pv_mda_free,pv_mda_size,pv_mda_count,pv_mda_used_count,pe_count --config="global{locking_type=0}"', name="pvs", alias="pvs")
pvs_noheadings = sf.simple_command("/sbin/pvs --nameprefixes --noheadings --separator='|' -a -o pv_all,vg_name --config=\"global{locking_type=0}\"", name="pvs_noheadings", alias="pvs_noheadings")
...
rc_local = sf.simple_file("/etc/rc.d/rc.local", name="rc_local", alias="rc_local")
redhat_release = sf.simple_file("/etc/redhat-release", name="redhat_release", alias="redhat-release")
resolv_conf = sf.simple_file("/etc/resolv.conf", name="resolve_conf", alias="resolve.conf")
...
samba = sf.simple_file("/etc/samba/smb.conf", name="samba", alias="samba")
...
scheduler = sf.foreach(block_devices, "/bin/cat /sys/block/%s/queue/scheduler", name="scheduler", alias="scheduler")
scsi = sf.simple_file("/proc/scsi/scsi", name="scsi", alias="scsi")
secure = sf.simple_file("/var/log/secure", name="secure", alias="secure")
selinux_config = sf.simple_file("/etc/selinux/config", name="selinux_config")
sestatus = sf.simple_command("/usr/sbin/sestatus -b", name="sestatus", alias="sestatus")
...
smartctl = sf.foreach(block, "/sbin/smartctl -a %s", name="smartctl", keep_rc=True, alias="smartctl")
spfile_ora = sf.glob_file("${ORACLE_HOME}/dbs/spfile*.ora", name="spfile_ora")
ss = sf.simple_command("/usr/sbin/ss -tulpn", name="ss", alias="ss")
ssh_config = sf.simple_file("/etc/ssh/ssh_config", name="ssh_config", alias="ssh_config")
sshd_config = sf.simple_file("/etc/ssh/sshd_config", name="sshd_config", alias="sshd_config")
...
sssd_logs = sf.glob_file("/var/log/sssd/*.log", name="sssd_logs", alias="sssd_logs")
...
sysctl = sf.simple_command("/sbin/sysctl -a", name="sysctl", alias="sysctl")
...
up2date = sf.simple_file("/etc/sysconfig/rhn/up2date", name="up2date", alias="up2date")
...
vgdisplay = sf.simple_command("/sbin/vgdisplay", name="vgdisplay", alias="vgdisplay")
vdsm_conf = sf.simple_file("etc/vdsm/vdsm.conf", name="vdsm_conf", alias="vdsm.conf")
vdsm_id = sf.simple_file("etc/vdsm/vdsm.id", name="vdsm_id", alias="vdsm_id")
...
woopsie = sf.simple_command(r"/usr/bin/find /var/crash /var/tmp -path '*.reports-*/whoopsie-report'", name="woopsie", alias="woopsie")
xfs_info = None
...
yum_repolist = sf.simple_command("/usr/bin/yum -C repolist", name="yum-repolist")
