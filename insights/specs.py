...
ceph_config_show = sf.with_args_from(ceph_socket_files, "/usr/bin/ceph daemon %s config show", name="ceph_socket_show")
...
ceph_osd_ec_profile_get = sf.with_args_from(ceph_osd_ec_profile_ls, "/usr/bin/ceph osd erasure-code-profile get %s -f json-pretty", name="ceph_osd_ec_profile_get")
...
docker_image_inspect = sf.with_args_from(docker_image_ids, "/usr/bin/docker inspect --type=image %s", name="docker_image_inspect", context=DockerHostContext)
docker_container_inspect = sf.with_args_from(docker_container_ids, "/usr/bin/docker inspect --type=container %s", name="docker_container_inspect", context=DockerHostContext)
...
ethtool_c = sf.with_args_from(ethernet_interfaces, "/sbin/ethtool -c %s", name="ethtool_c")
ethtool_g = sf.with_args_from(ethernet_interfaces, "/sbin/ethtool -g %s", name="ethtool_g")
...
httpd_pid = sf.simple_command("ps aux | grep asf | grep -v grep | head -1 | awk '{print $2}'", name="httpd_pid")
...
ipcs_s_i = sf.with_args_from(semid, "/usr/bin/ipcs -s -i %s", name="ipcs_s_i")
...
modinfo = sf.with_args_from(module, "/usr/sbin/modinfo %s", name="modinfo")
...
openshift_certificates = sf.with_args_from(crt, "/usr/bin/openssl x509 -noout -enddate -in %s", name="openshift_certificates")
