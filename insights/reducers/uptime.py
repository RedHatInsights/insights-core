...
Shared reducer for uptime information. It uses the results of the ``Uptime``
mapper and the ``Facter`` mapper to get the uptime information.  ``Uptime`` is
...
from insights.mappers.uptime import Uptime as upt
from insights.mappers.facter import Facter
...
@reducer(requires=[[upt, Facter]], shared=True)
def uptime(local, shared):
    ...
    Raises:
        Exception: If nothing can be gotten from both the two mappers.
    """
