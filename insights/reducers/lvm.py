"""
Lvm - Shared reducer for lvm information
========================================

This shared reducer for LVM mappers consolidates all of the information for
...
The mappers gather this information from multiple locations such as Insights
...
Sample input data for LVS commands as parsed by the mappers::
...
from insights.mappers.lvm import Lvs, LvsHeadings, Pvs, PvsHeadings, Vgs, VgsHeadings


def get_shared_data(mapper, shared):
    """
    Returns the actual list of mapper data based on how data is
    stored in mapper, either from the `data` attribute or from the
    `data['content']` attribute.

    Returns:
        list: List of mapper data.
    """
    if mapper in shared:
        return (copy.deepcopy(shared[mapper].data)
                if 'content' not in shared[mapper].data
                else copy.deepcopy(shared[mapper].data['content']))
    else:
        return []
...
@reducer(optional=[Lvs, LvsHeadings, Pvs, PvsHeadings, Vgs, VgsHeadings], shared=True)
class Lvm(object):
    """Class implements shared reducer for LVM information."""
