...
class DefaultSpecs(Specs):
    ...
    azure_instance_type = simple_command("/usr/bin/curl -s -H Metadata:true 'http://169.254.169.254/metadata/instance/compute/vmSize?api-version=2021-01-01&format=text' --connect-timeout 5", deps=[IsAzure])
    azure_instance_plan = simple_command("/usr/bin/curl -s -H Metadata:true 'http://169.254.169.254/metadata/instance/compute/plan?api-version=2021-02-01&format=json' --connect-timeout 5", deps=[IsAzure])
