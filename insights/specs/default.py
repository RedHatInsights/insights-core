...
class DefaultSpecs(Specs):
    ...
    ceph_osd_log = glob_file("/var/log/ceph/ceph-osd*.log")
    ...
    cobbler_settings = simple_file("/etc/cobbler/settings")
    cobbler_modules_conf = simple_file("/etc/cobbler/modules.conf")
    ...
    krb5 = glob_file(["/etc/krb5.conf", "/etc/krb5.conf.d/*.conf"])
    ...
    meminfo = simple_file("/proc/meminfo")
    ...
    osa_dispatcher_log = simple_file("/var/log/rhn/osa-dispatcher.log")
    ...
    rhn_conf = simple_file("/etc/rhn/rhn.conf")
    rhn_entitlement_cert_xml = glob_file("/etc/sysconfig/rhn/rhn-entitlement-cert.xml*")
    rhn_hibernate_conf = simple_file("/usr/share/rhn/config-defaults/rhn_hibernate.conf")
    ...
    rhn_server_xmlrpc_log = simple_file("/var/log/rhn/rhn_server_xmlrpc.log")
    rhn_search_daemon_log = simple_file("/var/log/rhn/search/rhn_search_daemon.log")
    rhn_taskomatic_daemon_log = simple_file("/var/log/rhn/rhn_taskomatic_daemon.log")
    ...
    systemid = simple_file("/etc/sysconfig/rhn/systemid")
    ...
    tomcat_web_xml = glob_file("/etc/tomcat*/web.xml")
    tomcat_server_xml = foreach_collect(tomcat_base, "%s/conf/server.xml")
    ...
    usr_journald_conf_d = glob_file("/usr/lib/systemd/journald.conf.d/*.conf")  # note that etc_journald.conf.d also exists
    ...
    virt_who_conf = glob_file(["/etc/virt-who.conf", "/etc/virt-who.d/*.conf"])
