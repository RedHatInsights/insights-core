...
class Specs(SpecSet):
    ...
    ansible_host = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    blacklisted_specs = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    branch_info = RegistryPoint(raw=True)  # https://issues.redhat.com/browse/RHIN-639
    display_name = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    tags = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    version_info = RegistryPoint(not_obfuscate=['hostname', 'ip'])

    # Regular collection specs
    abrt_ccpp_conf = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    abrt_status_bare = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    alternatives_display_python = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    amq_broker = RegistryPoint(multi_output=True)
    audit_log = RegistryPoint(filterable=True)
    auditctl_rules = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    auditctl_status = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    azure_instance_id = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    azure_instance_plan = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    azure_instance_type = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    azure_load_balancer = RegistryPoint()
    basic_auth_insights_client = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    bdi_read_ahead_kb = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    bios_uuid = RegistryPoint()
    blkid = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    bond = RegistryPoint(multi_output=True)
    bond_dynamic_lb = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ...
    cciss = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ...
    certificates_enddate = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    cgroups = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    checkin_conf = RegistryPoint()
    chkconfig = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    cpe = RegistryPoint()
    cpu_cores = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    cpu_siblings = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    cpu_smt_active = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    cpu_smt_control = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    cpu_vulns = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    cpuinfo = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    cpuinfo_max_freq = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    cpupower_frequency_info = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    cpuset_cpus = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    crt = RegistryPoint()
    crypto_policies_bind = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    crypto_policies_config = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    crypto_policies_opensshserver = RegistryPoint()
    crypto_policies_state_current = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    cryptsetup_luksDump = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    cupsd_conf = RegistryPoint()
    cups_files_conf = RegistryPoint()
    cups_ppd = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    current_clocksource = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    date = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    date_iso = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    date_utc = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    db2ls_a_c = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    df__li = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    dig = RegistryPoint()
    ...
    dnf_modules = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ...
    dotnet_version = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    doveconf = RegistryPoint(filterable=True)
    dracut_kdump_capture_service = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    dse_ldif = RegistryPoint(multi_output=True, filterable=True)
    du_dirs = RegistryPoint(multi_output=True, filterable=True, not_obfuscate=['hostname', 'ip'])
    ...
    etc_machine_id = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    ethtool = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ethtool_S = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ethtool_T = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ethtool_a = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ethtool_c = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ethtool_g = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ethtool_i = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ethtool_k = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    exim_conf = RegistryPoint()
    facter = RegistryPoint()
    fapolicyd_rules = RegistryPoint(multi_output=True, filterable=True, not_obfuscate=['hostname', 'ip'])
    fc_match = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    fcoeadm_i = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    fdisk_l = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    fdisk_l_sos = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    findmnt_lo_propagation = RegistryPoint()
    firewall_cmd_list_all_zones = RegistryPoint()
    firewalld_conf = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    ...
    foreman_rake_db_migrate_status = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    gcp_instance_type = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    gcp_license_codes = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    gcp_network_interfaces = RegistryPoint()
    getcert_list = RegistryPoint()
    getconf_page_size = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    getenforce = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    getsebool = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    group_info = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    grub1_config_perms = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    grub2_cfg = RegistryPoint()
    grub2_efi_cfg = RegistryPoint()
    grub_conf = RegistryPoint()
    grub_config_perms = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    grub_efi_conf = RegistryPoint()
    grubby_default_index = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    grubby_default_kernel = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    heat_crontab = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    heat_crontab_container = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    heat_engine_log = RegistryPoint(filterable=True)
    hostname = RegistryPoint(not_obfuscate=['ip'])
    hostname_default = RegistryPoint(not_obfuscate=['ip'])
    hostname_short = RegistryPoint(not_obfuscate=['ip'])
    hosts = RegistryPoint()
    hponcfg_g = RegistryPoint()
    httpd24_httpd_error_log = RegistryPoint(filterable=True)
    httpd_M = RegistryPoint(multi_output=True)
    httpd_V = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    httpd_access_log = RegistryPoint(filterable=True)
    httpd_cert_info_in_nss = RegistryPoint(multi_output=True, filterable=True, not_obfuscate=['ip'])
    ...
    httpd_limits = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    httpd_on_nfs = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    httpd_ssl_access_log = RegistryPoint(filterable=True)
    httpd_ssl_cert_enddate = RegistryPoint(multi_output=True, not_obfuscate=['ip'])
    httpd_ssl_error_log = RegistryPoint(filterable=True)
    ibm_fw_vernum_encoded = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ibm_lparcfg = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    ifcfg = RegistryPoint(multi_output=True)
    ifcfg_static_route = RegistryPoint(multi_output=True)
    ifconfig = RegistryPoint()
    imagemagick_policy = RegistryPoint(multi_output=True, filterable=True, not_obfuscate=['ip'])
    init_ora = RegistryPoint()
    init_process_cgroup = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    initctl_lst = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    initscript = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    insights_client_conf = RegistryPoint(filterable=True)
    installed_rpms = RegistryPoint(not_obfuscate=['ip'])
    interrupts = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ip6tables = RegistryPoint()
    ip6tables_permanent = RegistryPoint()
    ip_addr = RegistryPoint(not_obfuscate=['hostname'])
    ip_addresses = RegistryPoint(not_obfuscate=['hostname'])
    ip_neigh_show = RegistryPoint(not_obfuscate=['hostname'])
    ip_netns_exec_namespace_lsof = RegistryPoint(multi_output=True, filterable=True, not_obfuscate=['hostname', 'ip'])
    ip_route_show_table_all = RegistryPoint(not_obfuscate=['hostname'])
    ip_s_link = RegistryPoint(not_obfuscate=['hostname'])
    ipa_default_conf = RegistryPoint()
    ipaupgrade_log = RegistryPoint(filterable=True)
    ipcs_m = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ipcs_m_p = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ipcs_s = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ipcs_s_i = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ipsec_conf = RegistryPoint(filterable=True)
    iptables = RegistryPoint()
    iptables_permanent = RegistryPoint()
    ipv4_neigh = RegistryPoint(not_obfuscate=['hostname'])
    ipv6_neigh = RegistryPoint(not_obfuscate=['hostname'])
    iris_cpf = RegistryPoint(multi_output=True)
    iris_list = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    jboss_runtime_versions = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    jboss_version = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    journal_all = RegistryPoint(filterable=True)
    journal_header = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    journal_since_boot = RegistryPoint(filterable=True)
    katello_service_status = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    kdump_conf = RegistryPoint()
    kerberos_kdc_log = RegistryPoint(filterable=True)
    kernel_config = RegistryPoint(multi_output=True, filterable=True, not_obfuscate=['hostname', 'ip'])
    kernel_crash_kexec_post_notifiers = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    kexec_crash_loaded = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    kexec_crash_size = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    keystone_conf = RegistryPoint()
    keystone_crontab = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    keystone_crontab_container = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    keystone_log = RegistryPoint(filterable=True)
    kpatch_list = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    krb5 = RegistryPoint(multi_output=True)
    ksmstate = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ktimer_lockless = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    kubepods_cpu_quota = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    lastupload = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    leapp_migration_results = RegistryPoint()
    leapp_report = RegistryPoint()
    ld_library_path_of_user = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    limits_conf = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    locale = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    localtime = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    logrotate_conf = RegistryPoint(multi_output=True)
    losetup = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lpfc_max_luns = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lpstat_p = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    lscpu = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lsinitrd = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    lsinitrd_kdump_image = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    lsinitrd_lvm_conf = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    lsmod = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lsof = RegistryPoint(filterable=True)
    lspci = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lspci_vmmkn = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lssap = RegistryPoint()
    lsscsi = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    luksmeta = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    lvdisplay = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lvm_conf = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    lvm_fullreport = RegistryPoint()
    lvm_system_devices = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lvmconfig = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lvs_headings = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lvs_noheadings = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    lvs_noheadings_all = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    mac_addresses = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    machine_id = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    manila_conf = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    mariadb_log = RegistryPoint(filterable=True)
    max_uid = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    md5chk_files = RegistryPoint(multi_output=True)
    mdadm_D = RegistryPoint()
    mdadm_E = RegistryPoint(multi_output=True)
    mdstat = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    meminfo = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    mlx4_port = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    modinfo_filtered_modules = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    modinfo_modules = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    modprobe = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    mokutil_sbstate = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    mongod_conf = RegistryPoint(multi_output=True, filterable=True)
    mount = RegistryPoint()
    mountinfo = RegistryPoint()
    mounts = RegistryPoint()
    mpirun_version = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    mssql_api_assessment = RegistryPoint()
    mssql_conf = RegistryPoint()
    mssql_tls_cert_enddate = RegistryPoint()
    multicast_querier = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    nmcli_conn_show = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    nova_uid = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    numa_cpus = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    numeric_user_group_name = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    nvme_core_io_timeout = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    od_cpu_dma_latency = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    os_release = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    package_provides_command = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    pam_conf = RegistryPoint()
    parted__l = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    proc_keys = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    proc_keyusers = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    proc_netstat = RegistryPoint()
    proc_slabinfo = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    proc_snmp_ipv4 = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    proc_snmp_ipv6 = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    proc_stat = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    puppet_ca_cert_expire_date = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    ql2xmaxlun = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ql2xmqsupport = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    readlink_e_etc_mtab = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    readlink_e_shift_cert_client = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    readlink_e_shift_cert_server = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    recvq_socket_buffer = RegistryPoint()
    redhat_release = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    repquota_agnpuv = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    sap_hdb_version = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ...
    saphostexec_status = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    saphostexec_version = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    sat5_insights_properties = RegistryPoint()
    satellite_compute_resources = RegistryPoint()
    satellite_content_hosts_count = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    satellite_core_taskreservedresource_count = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    satellite_logs_table_size = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    satellite_qualified_katello_repos = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    satellite_rhv_hosts_count = RegistryPoint()
    satellite_sca_status = RegistryPoint()
    satellite_settings = RegistryPoint()
    satellite_version_rb = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    scsi_mod_max_report_luns = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    scsi_mod_use_blk_mq = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    sctp_asc = RegistryPoint()
    sctp_eps = RegistryPoint()
    sctp_snmp = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    sealert = RegistryPoint()
    secure = RegistryPoint(filterable=True)
    selinux_config = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    selinux_users = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    sendmail_mc = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    sendq_socket_buffer = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    sestatus = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    setup_named_chroot = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    smartctl = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    smartpdc_settings = RegistryPoint(filterable=True)
    smbstatus_S = RegistryPoint()
    smbstatus_p = RegistryPoint()
    sockstat = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    softnet_stat = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    software_collections_list = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    sshd_config_perms = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    subscription_manager_id = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    subscription_manager_installed_product_ids = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    subscription_manager_list_consumed = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    subscription_manager_list_installed = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    subscription_manager_release_show = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    sys_fs_cgroup_memory_tasks_number = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    sys_fs_cgroup_uniq_memory_swappiness = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    sys_kernel_sched_features = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    sys_vmbus_class_id = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    sys_vmbus_device_id = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    ...
    systemctl_list_unit_files = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    systemctl_list_units = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    thp_enabled = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    vdsm_id = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    vhost_net_zero_copy_tx = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    virsh_list_all = RegistryPoint()
    virt_uuid_facts = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    virt_what = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    virt_who_conf = RegistryPoint(multi_output=True, filterable=True)
    virtlogd_conf = RegistryPoint(filterable=True)
    vma_ra_enabled = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    ...
    x86_ibpb_enabled = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    x86_ibrs_enabled = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    x86_pti_enabled = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    x86_retp_enabled = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    xfs_db_frag = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    xfs_db_freesp = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    xfs_info = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    xfs_quota_state = RegistryPoint(not_obfuscate=['hostname', 'ip'])
    xinetd_conf = RegistryPoint(multi_output=True)
    yum_conf = RegistryPoint()
    yum_list_available = RegistryPoint(not_obfuscate=['ip'])
    yum_list_installed = RegistryPoint(not_obfuscate=['ip'])
    yum_log = RegistryPoint(filterable=True, not_obfuscate=['ip'])
    yum_repolist = RegistryPoint()
    yum_repos_d = RegistryPoint(multi_output=True)
    yum_updateinfo = RegistryPoint(not_obfuscate=['ip'])
    yum_updates = RegistryPoint(not_obfuscate=['ip'])
    zdump_v = RegistryPoint()
    zipl_conf = RegistryPoint()

    # container_specs
    container_cpu_online = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    container_cpuset_cpus = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    container_dotnet_version = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    container_installed_rpms = RegistryPoint(multi_output=True, not_obfuscate=['ip'])
    container_inspect_keys = RegistryPoint(filterable=True, not_obfuscate=['hostname', 'ip'])
    ...
    container_redhat_release = RegistryPoint(multi_output=True, not_obfuscate=['hostname', 'ip'])
    container_vsftpd_conf = RegistryPoint(multi_output=True, filterable=True)
    containers_inspect = RegistryPoint(not_obfuscate=['hostname', 'ip'])
