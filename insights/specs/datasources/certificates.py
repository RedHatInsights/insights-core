"""
Custom datasource for certificates_enddate
"""
import logging
...
def get_certificate_enddate(ctx, path):
    """
    ...
    if os.path.isdir(path):
        for dirpath, dirnames, filenames in os.walk(path):
            for fn in filenames:
                ret.extend(get_it(os.path.join(dirpath, fn)))
    elif os.path.isfile(path):
        ret.extend(get_it(path))
    return ret
...
def cert_and_path(broker):
    """
    Collect a list of certificate_enddate information and the path to each
    ...
    """
    paths = get_filters(Specs.certificates_enddate)
    ...
    if paths:
        ...
        if cert_path:
            return DatasourceProvider('\n'.join(cert_path), relative_path='insights_commands/certificates_enddate')
    raise SkipComponent
