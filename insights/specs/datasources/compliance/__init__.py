...
class ComplianceClient:
    ...
    def fetch_tailoring_ssg_version(self, policy):
        """Fetch the security_guide_version for the tailoring assigned to this policy.

        Every policy always has a tailoring (even when no rules have been customised),
        so a non-200 response or a missing version field is treated as an error.
        Transient failures are retried up to TAILORING_FETCH_ATTEMPTS times.
        """
        url = "{0}/compliance/v2/policies/{1}/tailorings/{2}".format(
        ...
        for attempt in range(1, TAILORING_FETCH_ATTEMPTS + 1):
            ...
            if 200 <= response.status_code < 300:
                break
            logger.warning(
                "Something went wrong fetching tailoring metadata for policy {0}. "
            ...
            logger.error(
                "Failed to fetch tailoring metadata for policy {0} after {1} attempts "
                "(last HTTP status {2}). Cannot verify SSG version compatibility.".format(
                    policy['ref_id'], TAILORING_FETCH_ATTEMPTS, response.status_code
                )
            )
            exit(constants.sig_kill_bad)
        return response.json().get('data', {}).get('security_guide_version')

    def verify_ssg_version(self, policy, backend_ssg_version):
        """Fail the scan when the backend's SSG version differs from the installed one.

        A mismatch means the SCAP datastreams on the system were generated by a
        different SSG release than the one the Compliance service used to build
        the tailoring, which can produce incorrect or incomplete results.
        """
        if backend_ssg_version != self.ssg_version:
            logger.error(
                "SSG version mismatch for policy {0}: "
                "the Compliance backend expects scap-security-guide {1} "
                "but {2} is installed on this system. "
                "Please install version {1} of the scap-security-guide package and retry.".format(
                    policy['ref_id'], backend_ssg_version, self.ssg_version
                )
            )
            exit(constants.sig_kill_bad)
