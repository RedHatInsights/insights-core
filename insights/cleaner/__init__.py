...
class Cleaner(object):
    ...
    def __init__(self, config, rm_conf, fqdn=None):
        ...
        if config and config.obfuscation_list:
            obf_opt = config.obfuscation_list
            # - IPv4 obfuscation
            self.obfuscate.update(ipv4=IPv4()) if 'ipv4' in obf_opt else None
            # - IPv6 obfuscation
            self.obfuscate.update(ipv6=IPv6()) if 'ipv6' in obf_opt else None
            # - Hostname obfuscation
            self.obfuscate.update(hostname=Hostname(self.fqdn)) if 'hostname' in obf_opt else None
            # - MAC obfuscation
            self.obfuscate.update(mac=Mac()) if 'mac' in obf_opt else None
