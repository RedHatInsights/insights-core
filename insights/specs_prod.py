...
class ProdSpecs(Specs):
    autofs_conf = simple_file("/etc/autofs.conf", alias="autofs.conf")
    audit_log = simple_file("/var/log/audit/audit.log")
    auditd_conf = simple_file("/etc/audit/auditd.conf", alias="auditd.conf")
    ...
    candlepin_log = simple_file("/var/log/candlepin/candlepin.log", alias="candlepin.log")
    ...
    ps_auxww = first_of([
                        ...
                        simple_file('sos_commands/process/ps_aux', alias='ps_aux', context=HostArchiveContext),
                        simple_file('sos_commands/process/ps_auxwww', alias='ps_auxwww', context=HostArchiveContext),
                        simple_file('sos_commands/process/ps_auxcww', alias='ps_auxcww', context=HostArchiveContext),
                        ])
    ...
    ceilometer_conf = simple_file("/etc/ceilometer/ceilometer.conf", alias="ceilometer.conf")
    ...
    ceph_osd_log = glob_file(r"var/log/ceph/ceph-osd*.log", alias="ceph_osd.log")
    ...
    chrony_conf = simple_file("/etc/chrony.conf", alias="chrony.conf")
    chronyc_sources = simple_command("/usr/bin/chronyc sources")
    cib_xml = simple_file("/var/lib/pacemaker/cib/cib.xml", alias="cib.xml")
    cinder_conf = simple_file("/etc/cinder/cinder.conf", alias="cinder.conf")
    cinder_volume_log = simple_file("/var/log/cinder/volume.log", alias="cinder_volume.log")
    cluster_conf = simple_file("/etc/cluster/cluster.conf", alias="cluster.conf")
    ...
    cobbler_modules_conf = first_file(["/etc/cobbler/modules.conf", "/conf/cobbler/modules.conf"], alias="cobbler_modules.conf")
    ...
    df__al = simple_command("/bin/df -al", alias="df_-al")
    df__alP = simple_command("/bin/df -alP", alias="df_-alP")
    df__li = simple_command("/bin/df -li", alias="df_-li")
    ...
    dumpe2fs_h = foreach_execute(dumpdev, "/sbin/dumpe2fs -h %s", alias="dumpe2fs-h")
    engine_log = simple_file("/var/log/ovirt-engine/engine.log", alias="engine.log")

    etc_journald_conf = simple_file(r"etc/systemd/journald.conf", alias="etc_journald.conf")
    etc_journald_conf_d = glob_file(r"etc/systemd/journald.conf.d/*.conf", alias="etc_journald.conf.d")
    ...
    ethtool_S = foreach_execute(ethernet_interfaces, "/sbin/ethtool -S %s", alias="ethtool-S")
    ethtool_a = foreach_execute(ethernet_interfaces, "/sbin/ethtool -a %s", alias="ethtool-a")
    ethtool_c = foreach_execute(ethernet_interfaces, "/sbin/ethtool -c %s", alias="ethtool-c")
    ethtool_g = foreach_execute(ethernet_interfaces, "/sbin/ethtool -g %s", alias="ethtool-g")
    ethtool_i = foreach_execute(ethernet_interfaces, "/sbin/ethtool -i %s", alias="ethtool-i")
    ethtool_k = foreach_execute(ethernet_interfaces, "/sbin/ethtool -k %s", alias="ethtool-k")
    exim_conf = simple_file("etc/exim.conf", alias="exim.conf")
    facter = simple_command("/usr/bin/facter")
    fc_match = simple_command("/bin/fc-match -sv 'sans:regular:roman' family fontformat", alias="fc-match")
    fdisk_l = simple_command("/sbin/fdisk -l", alias="fdisk-l")
    fdisk_l_sos = glob_file(r"sos_commands/filesys/fdisk_-l_*", context=HostArchiveContext)
    foreman_production_log = simple_file("/var/log/foreman/production.log", alias="foreman_production.log")
    foreman_proxy_conf = simple_file("/etc/foreman-proxy/settings.yml")
    foreman_proxy_log = simple_file("/var/log/foreman-proxy/proxy.log", alias="foreman_proxy.log")
    foreman_satellite_log = simple_file("/var/log/foreman-installer/satellite.log", alias="foreman_satellite.log")
    foreman_ssl_access_ssl_log = first_of([simple_file("var/log/httpd/foreman-ssl_access_ssl.log"), simple_file(r"sos_commands/foreman/foreman-debug/var/log/httpd/foreman-ssl_access_ssl.log", context=HostArchiveContext)], alias="foreman-ssl_access_ssl.log")
    fstab = simple_file("/etc/fstab")
    galera_cnf = simple_file("/etc/my.cnf.d/galera.cnf", alias="galera.cnf")
    ...
    glance_api_conf = simple_file("/etc/glance/glance-api.conf", alias="glance-api.conf")
    glance_api_log = simple_file("/var/log/glance/api.log")
    glance_cache_conf = simple_file("/etc/glance/glance-cache.conf", alias="glance-cache.conf")
    glance_registry_conf = simple_file("/etc/glance/glance-registry.conf", alias="glance-registry.conf")
    grub_conf = simple_file("/boot/grub/grub.conf", alias="grub.conf")
    grub_efi_conf = simple_file("/boot/efi/EFI/redhat/grub.conf", alias="grub-efi.conf")
    grub2_cfg = simple_file("/boot/grub2/grub.cfg", alias="grub2.cfg")
    grub2_efi_cfg = simple_file("boot/efi/EFI/redhat/grub.cfg", alias="grub2-efi.cfg")
    ...
    heat_api_log = simple_file("/var/log/heat/heat-api.log", alias="heat-api.log")
    heat_conf = simple_file("/etc/heat/heat.conf", alias="heat.conf")
    heat_crontab = simple_command("/usr/bin/crontab -l -u heat")
    heat_engine_log = simple_file("/var/log/heat/heat-engine.log", alias="heat-engine.log")
    hostname = simple_command("/usr/bin/hostname -f")
    hosts = simple_file("/etc/hosts")
    hponcfg_g = simple_command("/sbin/hponcfg -g", alias="hponcfg-g")
    ...
    httpd_V = foreach_execute(httpd_cmd, "%s -V", alias="httpd-V")
    ...
    init_ora = simple_file("${ORACLE_HOME}/dbs/init.ora", alias="init.ora")
    ...
    ip_s_link = simple_command("/sbin/ip -s link", alias="ip-s_link")
    ...
    katello_service_status = simple_command("/usr/bin/katello-service status", alias="katello-service_status")
    kdump = simple_file("/etc/sysconfig/kdump")
    kdump_conf = simple_file("/etc/kdump.conf", alias="kdump.conf")
    ...
    keystone_conf = simple_file("/etc/keystone/keystone.conf", alias="keystone.conf")
    keystone_crontab = simple_command("/usr/bin/crontab -l -u keystone")
    keystone_log = simple_file("/var/log/keystone/keystone.log", alias="keystone.log")
    ...
    libvirtd_log = simple_file("/var/log/libvirt/libvirtd.log", alias="libvirtd.log")
    ...
    lvm_conf = simple_file("/etc/lvm/lvm.conf", alias="lvm.conf")
    ...
    machine_id = simple_file("etc/redhat-access-insights/machine-id", alias="machine-id")
    manila_conf = simple_file("/etc/manila/manila.conf")
    mariadb_log = simple_file("/var/log/mariadb/mariadb.log", alias="mariadb.log")
    ...
    modprobe_conf = simple_file("/etc/modprobe.conf", alias="modprobe.conf")
    ...
    modprobe_d = glob_file("/etc/modprobe.d/*.conf", alias="modprobe.d")
    ...
    multipath_conf = simple_file("/etc/multipath.conf", alias="multipath.conf")
    multipath__v4__ll = simple_command("/sbin/multipath -v4 -ll", alias="multipath_-v4_-ll")
    named_checkconf_p = simple_command("/usr/sbin/named-checkconf -p", alias="named-checkconf_p")
    netconsole = simple_file("/etc/sysconfig/netconsole")
    netstat = simple_command("/bin/netstat -neopa")
    netstat_agn = simple_command("/bin/netstat -agn", alias="netstat_-agn")
    netstat_i = simple_command("/bin/netstat -i", alias="netstat-i")
    netstat_s = simple_command("/bin/netstat -s", alias="netstat-s")
    neutron_conf = simple_file("/etc/neutron/neutron.conf", alias="neutron.conf")
    neutron_ovs_agent_log = simple_file("/var/log/neutron/openvswitch-agent.log")
    neutron_plugin_ini = simple_file("/etc/neutron/plugin.ini", alias="neutron_plugin.ini")
    ...
    nfs_exports_d = glob_file("/etc/exports.d/*.exports", alias="nfs_exports.d")
    nginx_conf = glob_file([
                           "/etc/nginx/nginx.conf",
                           "/opt/rh/nginx*/root/etc/nginx/nginx.conf",
                           "/etc/opt/rh/rh-nginx*/nginx/nginx.conf"
                           ])
    nova_api_log = simple_file("/var/log/nova/nova-api.log", alias="nova-api_log")
    nova_compute_log = simple_file("/var/log/nova/nova-compute.log", alias="nova-compute.log")
    nova_conf = simple_file("/etc/nova/nova.conf", alias="nova.conf")
    nova_crontab = simple_command("/usr/bin/crontab -l -u nova")
    nscd_conf = simple_file("/etc/nscd.conf", alias="nscd.conf")
    nsswitch_conf = simple_file("/etc/nsswitch.conf", alias="nsswitch.conf")
    ntp_conf = simple_file("/etc/ntp.conf", alias="ntp.conf")
    ...
    ovirt_engine_server_log = simple_file("/var/log/ovirt-engine/server.log", alias="ovirt_engine_server.log")
    ovs_vsctl_show = simple_command("/usr/bin/ovs-vsctl show", alias="ovs-vsctl_show")
    pacemaker_log = simple_file("/var/log/pacemaker.log", alias="pacemaker.log")
    ...
    pam_conf = simple_file("/etc/pam.conf", alias="pam.conf")
    parted__l = simple_command("/sbin/parted -l -s", alias="parted_-l")
    password_auth = simple_file("/etc/pam.d/password-auth", alias="password-auth")
    pcs_status = simple_command("/usr/sbin/pcs status")
    pluginconf_d = glob_file("/etc/yum/pluginconf.d/*.conf", alias="pluginconf.d")
    ...
    postgresql_log = first_of([
                              ...
                              glob_file("/database/postgresql-*.log", alias="postgresql.log")
                              ])
    ...
    qemu_conf = simple_file("/etc/libvirt/qemu.conf", alias="qemu.conf")
    ...
    rc_local = simple_file("/etc/rc.d/rc.local", alias="rc.local")
    redhat_release = simple_file("/etc/redhat-release", alias="redhat-release")
    resolv_conf = simple_file("/etc/resolv.conf", alias="resolv.conf")
    rhn_charsets = simple_command("/usr/bin/rhn-charsets", alias="rhn-charsets")
    rhn_conf = first_file(["/etc/rhn/rhn.conf", "/conf/rhn/rhn/rhn.conf"], alias="rhn.conf")
    rhn_entitlement_cert_xml = first_of([glob_file("/etc/sysconfig/rhn/rhn-entitlement-cert.xml*"),
                                   ...
                                   alias="rhn-entitlement-cert.xml")
    rhn_hibernate_conf = first_file(["/usr/share/rhn/config-defaults/rhn_hibernate.conf", "/config-defaults/rhn_hibernate.conf"], alias="rhn_hibernate.conf")
    rhn_schema_stats = simple_command("/usr/bin/rhn-schema-stats -", alias="rhn-schema-stats")
    rhn_schema_version = simple_command("/usr/bin/rhn-schema-version", alias="rhn-schema-version")
    rhn_server_satellite_log = simple_file("var/log/rhn/rhn_server_satellite.log", alias="rhn_server_satellite.log")
    rhn_server_xmlrpc_log = first_file(["/var/log/rhn/rhn_server_xmlrpc.log",
                                           "/rhn-logs/rhn/rhn_server_xmlrpc.log"],
                                           alias="rhn_server_xmlrpc.log")
    rhn_search_daemon_log = first_file(["/var/log/rhn/search/rhn_search_daemon.log",
                                           "/rhn-logs/rhn/search/rhn_search_daemon.log"],
                                           alias="rhn_search_daemon.log")
    rhn_taskomatic_daemon_log = first_file(["/var/log/rhn/rhn_taskomatic_daemon.log",
                                               "rhn-logs/rhn/rhn_taskomatic_daemon.log"],
                                               alias="rhn_taskomatic_daemon.log")
    rhsm_conf = simple_file("/etc/rhsm/rhsm.conf", alias="rhsm.conf")
    rhsm_log = simple_file("/var/log/rhsm/rhsm.log", alias="rhsm.log")
    root_crontab = simple_command("/usr/bin/crontab -l -u root")
    route = simple_command("/sbin/route -n")
    rpm_V_packages = simple_command("/usr/bin/rpm -V coreutils procps procps-ng shadow-utils passwd sudo", alias="rpm_-V_packages")
    rsyslog_conf = simple_file("/etc/rsyslog.conf", alias="rsyslog.conf")
    samba = simple_file("/etc/samba/smb.conf")
    satellite_version_rb = simple_file("/usr/share/foreman/lib/satellite/version.rb", alias="satellite_version.rb")
    ...
    selinux_config = simple_file("/etc/selinux/config", alias="selinux-config")
    ...
    spfile_ora = glob_file("${ORACLE_HOME}/dbs/spfile*.ora", alias="spfile.ora")
    ...
    sysctl_conf = simple_file("/etc/sysctl.conf", alias="sysctl.conf")
    sysctl_conf_initramfs = simple_command("/bin/lsinitrd /boot/initramfs-*kdump.img -f /etc/sysctl.conf /etc/sysctl.d/*.conf", alias="sysctl.conf_initramfs")
    systemctl_cinder_volume = simple_command("/bin/systemctl show openstack-cinder-volume", alias="systemctl_cinder-volume")
    systemctl_list_unit_files = simple_command("/bin/systemctl list-unit-files", alias="systemctl_list-unit-files")
    systemctl_list_units = simple_command("/bin/systemctl list-units", alias="systemctl_list-units")
    ...
    systemd_system_conf = simple_file("/etc/systemd/system.conf", alias="systemd_system.conf")
    systemid = first_of([
    ...
    tuned_adm = simple_command("/usr/sbin/tuned-adm list", alias="tuned-adm")
    udev_persistent_net_rules = simple_file("/etc/udev/rules.d/70-persistent-net.rules", alias="udev-persistent-net.rules")
    ...
    usr_journald_conf_d = glob_file(r"usr/lib/systemd/journald.conf.d/*.conf", alias="usr_journald.conf.d")  # note that etc_journald.conf.d also exists
    vgdisplay = simple_command("/sbin/vgdisplay")
    vdsm_conf = simple_file("etc/vdsm/vdsm.conf", alias="vdsm.conf")
    vdsm_id = simple_file("etc/vdsm/vdsm.id")
    vdsm_log = simple_file("var/log/vdsm/vdsm.log", alias="vdsm.log")
    ...
    virt_what = simple_command("/usr/sbin/virt-what", alias="virt-what")
    ...
    vsftpd_conf = simple_file("/etc/vsftpd/vsftpd.conf", alias="vsftpd.conf")
    woopsie = simple_command(r"/usr/bin/find /var/crash /var/tmp -path '*.reports-*/whoopsie-report'")
    xfs_info = None
    xinetd_conf = glob_file(["/etc/xinetd.conf", "/etc/xinetd.d/*"], alias="xinetd.conf")
    yum_conf = simple_file("/etc/yum.conf", alias="yum.conf")
    yum_log = simple_file("/var/log/yum.log", alias="yum.log")
    yum_repolist = simple_command("/usr/bin/yum -C repolist", alias="yum-repolist")
    yum_repos_d = glob_file("/etc/yum.repos.d/*", alias="yum.repos.d")
    ...
    installed_rpms = first_of([host_installed_rpms, docker_installed_rpms], alias="installed-rpms")
