...
def test_ps_auxcww():
    with warnings.catch_warnings(record=True) as w:
        ...
        assert sorted([c['COMMAND'] for c in p]) == sorted([
            'init', 'kondemand/0', 'irqbalance', 'bash', 'bash', 'bash', 'dhclient',
            'qemu-kvm', 'vdsm'
        ])
        assert 'sshd' not in p
        assert not p.fuzzy_match("sshd")
...
def test_ps_auxwww():
    with warnings.catch_warnings(record=True) as w:
        p = ps.PsAuxwww(context_wrap(PsAux_TEST))
        assert p.data[0] == {
            '%MEM': '0.0', 'TTY': '?', 'VSZ': '19356', 'PID': '1', '%CPU': '0.0',
            'START': 'May31', 'COMMAND': '/sbin/init', 'USER': 'root',
            'STAT': 'Ss', 'TIME': '0:01', 'RSS': '1544'
...
def test_ps_auxcww_pid():
    with warnings.catch_warnings(record=True):
        ...
        assert ps_auxcww.running_pids() == ['1', '1821', '1864', '20160', '20161', '20164', '20357', '22673', '27323']
...
def test_ps_auxcww_alternate():
    with warnings.catch_warnings(record=True):
        ...
        assert ps_auxcww.services == [
            ...
            ('bash', 'user2', 'user2    20161  0.0  0.0 108472  1896 pts/4    Ss   10:09   0:00 bash'),
            ('bash', 'user2', 'user2    20164  0.0  0.0 108472  1896 pts/5    Ss   10:10   0:00 bash'),
