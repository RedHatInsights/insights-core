...
def test_nftables_rulelist_exp():
    with pytest.raises(SkipException) as exc:
        NFTListRules(context_wrap(NFT_NO))
    assert 'No Contents' in str(exc.value)

    with pytest.raises(ParseException) as exc:
        NFTListRules(context_wrap(NFT_NO_1))
    assert 'No Parsed Contents' in str(exc.value)

    with pytest.raises(ParseException) as exc:
        NFTListRules(context_wrap(NFT_NO_2))
    assert 'Content out of table or chain' in str(exc.value)


def test_modinfo_doc_examples():
    env = {'nftables': NFTListRules(context_wrap(NFT_DOC))}
