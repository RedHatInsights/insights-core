...
def test_podman_list_images():
    result = docker_list.PodmanListImages(context_wrap(PODMAN_LIST_IMAGES))
    # All rows get read:
    assert len(result.rows) == 4
    # Rows with data are as normal
    assert result.rows[0].get("REPOSITORY") == "rhel6_vsftpd"
...
def test_podman_list_containers():
    result = docker_list.PodmanListContainers(context_wrap(PODMAN_LIST_CONTAINERS))
    assert len(result.rows) == 2
    assert result.rows[0].get("CONTAINER ID") == "03e2861336a76e29155836113ff6560cb70780c32f95062642993b2b3d0fc216"
    assert result.rows[0].get("COMMAND") == '"/usr/sbin/httpd -DFOREGROUND"'
    assert result.rows[0].get("SIZE") == "796 B (virtual 669.2 MB)"
...
def test_podman_list_images_no_data():
    with pytest.raises(SkipException) as ex:
        docker_list.DockerList(context_wrap(PODMAN_LIST_IMAGES_NO_DATA))
    assert 'No data.' in str(ex)


def test_podman_undefined_key_field():
    assert docker_list.DockerList(context_wrap(PODMAN_LIST_CONTAINERS)).key_field is None


def test_podman_documentation():
    failed_count, tests = doctest.testmod(
        docker_list,
        globs={'images': docker_list.PodmanListImages(context_wrap(PODMAN_LIST_IMAGES)),
               'containers': docker_list.PodmanListContainers(context_wrap(PODMAN_LIST_CONTAINERS))}
    )
    assert failed_count == 0
