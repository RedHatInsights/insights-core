...
%s
type=CRED_REFR msg=audit(1508476956.471:13339): pid=30909 uid=0 auid=0 ses=1559 subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 msg='op=PAM:setcred grantors=pam_unix acct="root" exe="/usr/sbin/sshd" hostname=ovpn-12-148.pek2.redhat.com addr=10.72.12.148 terminal=ssh res=success'
""".strip()
...
LAST_LINE_OF_TEMPLATE = """type=CRED_REFR msg=audit(1508476956.471:13339): pid=30909 uid=0 auid=0 ses=1559 subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 msg='op=PAM:setcred grantors=pam_unix acct="root" exe="/usr/sbin/sshd" hostname=ovpn-12-148.pek2.redhat.com addr=10.72.12.148 terminal=ssh res=success'"""

AuditLog.filters.extend([
    "CRYPTO_KEY_USER",
    "LOGIN"
])
...
def test_audit_log():
    ...
    assert line0.get('msg') == 'op=destroy kind=session fp=? direction=both spid=16865 suid=0 rport=59296 laddr=192.0.2.1 lport=22  exe="/usr/sbin/sshd" hostname=? addr=192.0.2.3 terminal=? res=success'
    ...
    assert logs[0] == {'timestamp': '1506487181.009',
    ...
    lines = (AUDIT_LOG_TEMPALTE % AUDIT_LOG_WITH_JUST_HEAD_PART).splitlines()
    logs = list(auditlog.get_after(timestamp=logtime, lines=lines))
    assert logs[0]['raw_message'] == AUDIT_LOG_WITH_JUST_HEAD_PART
    ...
    for option in options:
        ...
        assert len(info) == 3
        line2 = info[2]
        assert line2.get('type') == 'CRED_REFR'
    ...
    assert len(info) == 1
    line0 = info[0]
    assert line0.get('type') == 'AVC'
    assert line0.get('unparsed') is None
    assert line0.get('timestamp') == '1506487181.009'
    assert line0.get('msg_ID') == '32794'
    assert line0.get('raw_message') == 'type=AVC msg=audit(1506487181.009:32794):'
    assert line0 == {'timestamp': '1506487181.009', 'msg_ID': '32794', 'type': 'AVC',
