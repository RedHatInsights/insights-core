...
class GrubbyDefaultIndex(CommandParser):
    ...
    def parse_content(self, content):
        ...
        if not default_index_line.strip().isdigit():
            raise ParseException('Invalid output: ', content)
            ...
            for idx in range(len(content) - 1, -1, -1):
                ...
                if line.startswith('/boot/') and '/boot/vmlinuz-' in line:
                    if not default_kernel_str:
                        ...
                        raise ParseException('Invalid output: duplicate kernel lines: ', content)

        if not default_kernel_str:
            raise ParseException('Invalid output: no kernel line: ', content)
        if len(default_kernel_str.split()) > 1:
            raise ParseException('Invalid output: unparsable kernel line: ', content)
        ...
        for _line in content:
            ...
            if k == "index":
                if v.isdigit():
                    ...
                    raise ParseException('Invalid index value: ', _line)
            elif k == "args":
