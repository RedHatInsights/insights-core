...
It looks for all the running domain-controller, host-controller and process-controller processes
...
The reason to keep pid as key here is that it's easier to get property for a certain JBoss process in insights plugin.
...
Raises:
    insights.parsers.ParseException: if there is no running JBoss domain-controller, host-controller and process-controller process
"""
    ...
    def parse_content(self, content):
        ...
        if len(content) == 0:
            raise ParseException("Error: ",
                                 'there is no running JBoss domain-controller, host-controller and process-controller process')
        l = content[0].split("|")
