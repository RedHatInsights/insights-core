...
from ..parsers import unsplit_lines, keyword_search
...
class PamDConf(Parser):
    ...
    Sample input::
        ...
        ...     pass
        >>> shared = {YourPamDConf: YourPamDConf(context_wrap(pam_sshd, path='/etc/pam.d/sshd'))}
    ...
    def search(self, **kwargs):
        ...
        for obj in self.data:
            ...
            row['control_flags'] = {cf.flag: cf.value for cf in obj.control_flags}
            search.append(row)
        # Now get the result, and just return the entry objects
        return [r['entry_obj'] for r in keyword_search(search, **kwargs)]
