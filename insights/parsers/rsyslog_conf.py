...
@parser(Specs.rsyslog_conf, requires=[redhat_release])
class RsyslogConf(Parser):
    ...
    def __init__(self, content, rhel):
        self.rhel = rhel
        if content.path and content.content:
            self.path = content.path
            self.parse_content(content.content)
        else:
            raise ParseException("No Proper Contents")

    def parse_content(self, content):
        if self.rhel.major in [6, 7]:
            ...
            raise SkipException("System is not a RHEL-6 or RHEL-7")
...
class RsyslogConf8(Parser):
    ...
    def parse_content(self, content):
        ...
        if self.rhel.major == 8:
            ...
            raise SkipException("System is not a RHEL-8")
    ...
    @property
    def rsyslog_path(self):
        """
        This will return the path of rsyslog.conf file on success else it will
