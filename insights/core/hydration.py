...
def create_context(path, context=None):
    ...
    if context is None:
        context = ctx
    elif context is not ctx:
        raise ContextException(
            "ExecutionContext missing match:\n  specified: '{0}.{1}'\n  identified: '{2}.{3}({4})'".format(
                context.__module__, context.__name__, ctx.__module__, ctx.__name__, common_path
            )
        )
