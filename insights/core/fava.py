...
def translate_vars(outer, retranslate_function, ctx):
    ...
    if 'vars' in outer:
        ...
        del new_outer['vars']
        return make_wrapper({key: translate_value(value, ctx) for key, value in new_vars.iteritems()},
...
def make_rule_function(rule_body):
    ...
    return FavaCode(
        Expression(
            body=Lambda(
                args=arguments(
                    args=[Name(id='local', ctx=Param()), Name(id='shared', ctx=Param())],
