...
def test_grubby():
    ...
    result = Grubby(grubby_info_all, grubby_default_index, None)
...
def test_grubby_ab():
    ...
    with pytest.raises(ParseException) as excinfo:
        Grubby(grubby_info_all, grubby_default_index, None)
...
def test_grubby_extend_with_grubenv():
    ...
    result = Grubby(grubby_info_all, grubby_default_index, grubenv)
    assert result.default_index == 0
    default_boot_entry_data = dict(
        index=0,
        kernel="/boot/vmlinuz-5.14.0-570.30.1.el9_6.x86_64",
    ...
    result = Grubby(grubby_info_all, grubby_default_index, grubenv_wo_params)
    assert result.default_index == 0
