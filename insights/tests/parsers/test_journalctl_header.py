import pytest
import doctest
from insights.parsers import SkipException, journalctl_header
...
def test_journalctl_header_valid():
    parser_result = JournalctlHeader(context_wrap(OUTPUT_VALID))
...
def test_journalctl_header_invalid():
    parser_result = JournalctlHeader(context_wrap(OUTPUT_INVALID))
    assert parser_result is not None
    assert parser_result.errors[0] == "Failed to write header:"


def test_content_empty():
    with pytest.raises(SkipException):
        JournalctlHeader(context_wrap(OUTPUT_EMPTY))


def test_losetup_doc_examples():
    env = {'journalctl_header': JournalctlHeader(context_wrap(OUTPUT_VALID_EXAMPLE))}
