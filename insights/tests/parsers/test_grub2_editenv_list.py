...
def test_doc_examples():
    env = {
        'grub2_editenv_list': grub2_editenv_list.Grub2EditenvList(context_wrap(CONTENT_WITH_TUNED_PARAMS))
    }
...
def test_with_tuned_params():
    results = grub2_editenv_list.Grub2EditenvList(context_wrap(CONTENT_WITH_TUNED_PARAMS))
...
def test_with_error():
    results = grub2_editenv_list.Grub2EditenvList(context_wrap(CONTENT_ERROR))
    assert results is not None
    assert results.error == ["/usr/bin/grub2-editenv: error: invalid environment block."]


def test_without_tuned_params():
    results = grub2_editenv_list.Grub2EditenvList(context_wrap(CONTENT_WITHOUT_TUNED_PARAMS))
    assert results is not None
    assert results.has_kernelopts
...
def test_r7():
    results = grub2_editenv_list.Grub2EditenvList(context_wrap(CONTENT_RHEL7))
    assert results is not None
