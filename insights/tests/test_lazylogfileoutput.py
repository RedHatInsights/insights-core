...
from insights.core import LazyLogFileOutput
from insights.core.exceptions import ParseException
from insights.tests import context_wrap


class FakeMessagesClass(LazyLogFileOutput):
...
def test_lazylogfileoutput_scanners():
    ...
    assert not hasattr(log, 'puppet_master_logs')
    assert not hasattr(log, 'kernel_logs')
    ...
    log.do_scan('middleware_exception_present')
    assert hasattr(log, 'middleware_exception_present')
    log.do_scan('cron_present')
...
def test_lazylogfileoutput_scanners_list():
    # Messages that are present can be kept
    ...
    FakeMessagesClass.keep_scan(
        'puppet_master_first_any_3', ['puppet-master', 'first'], num=3, check=any
    )
    # Token scan of something that's absent
    ...
    FakeMessagesClass.last_scan(
        'puppet_master_first_last_any', ['puppet-master', 'first'], check=any
    )

    ctx = context_wrap(MESSAGES, path='/var/log/messages')
    log = FakeMessagesClass(ctx)

    assert not hasattr(log, 'puppet_master_manifest_logs')
    assert not hasattr(log, 'error_missing')
    assert not hasattr(log, 'puppet_master_first')
    assert not hasattr(log, 'error_info')
    assert not hasattr(log, 'puppet_master_first_any')
    assert not hasattr(log, 'puppet_master_first_any_3')
    assert not hasattr(log, 'error_info_any')
    assert not hasattr(log, 'puppet_master_manifest_last')
    assert not hasattr(log, 'puppet_master_first_last_any')
    # scan all and load content
    log.do_scan()
...
class FakeAccessLog(LazyLogFileOutput):
...
class FakeMariaDBLog(LazyLogFileOutput):
    time_format = ['%y%m%d %H:%M:%S', '%Y-%m-%d %H:%M:%S']


class DictMariaDBLog(LazyLogFileOutput):
    time_format = {'old': '%y%m%d %H:%M:%S', 'new': '%Y-%m-%d %H:%M:%S'}


class BadClassMariaDBLog(LazyLogFileOutput):
...
class FakeTowerLog(LazyLogFileOutput):
