from insights.combiners import grub_conf as grub_combiner
...
initrd /initramfs-4.18.0-80.1.2.el8_0.x86_64.img
options root=/dev/mapper/rhel_vm37--146-root ro crashkernel=auto resume=/dev/mapper/rhel_vm37--146-swap rd.lvm.lv=rhel_vm37-146/root rd.lvm.lv=rhel_vm37-146/swap noapic
...
BOOT_LOADER_ENTRIES_VARs = """
title Red Hat Enterprise Linux (4.18.0-32.el8.x86_64) 8.0 (Ootpa)
version 4.18.0-32.el8.x86_64
linux /vmlinuz-4.18.0-32.el8.x86_64
initrd /initramfs-4.18.0-32.el8.x86_64.img $tuned_initrd
...
def test_grub2_boot_loader_entries():
    grub_cfg = Grub2Config(context_wrap(GRUB2_RHEL8))
...
def test_grub2_boot_loader_entries_efi():
    grub_cfg = Grub2Config(context_wrap(GRUB2_RHEL8))
...
def test_grub2_boot_loader_entries_with_variables():
    grub_cfg = Grub2Config(context_wrap(GRUB2_RHEL8))
    grub_env = Grub2Grubenv(context_wrap(GRUB2_ENV))
    grub_ble1 = BLE(context_wrap(BOOT_LOADER_ENTRIES_1))
    grub_ble2 = BLE(context_wrap(BOOT_LOADER_ENTRIES_2))
    ...
    assert len(result.get_grub_cmdlines()) == 3
    assert 'noapic' in result.get_grub_cmdlines()[0]['cmdline']
    assert 'kernelopts' in grub_ble3['options']
...
def test_combiner_grub_conf_doc():
    grub_cfg = Grub2Config(context_wrap(GRUB2_RHEL8))
    grub_env = Grub2Grubenv(context_wrap(GRUB2_ENV))
    grub_ble1 = BLE(context_wrap(BOOT_LOADER_ENTRIES_1))
    grub_ble2 = BLE(context_wrap(BOOT_LOADER_ENTRIES_2))
    ...
    rpms = InstalledRpms(context_wrap(INSTALLED_RPMS_V2))
    sys_firmware = LsSysFirmware(context_wrap(SYS_FIRMWARE_DIR_NOEFI))
    grub_conf = GrubConf(None, grub_cfg, None, None, grub_bles, rpms, None, sys_firmware, rhel)
    env = {
            'grub_env': grub_env,
            'grub_cfg': grub_cfg,
            'grub_bles': grub_bles,
    ...
    failed, total = doctest.testmod(grub_combiner, globs=env)
    assert failed == 0
