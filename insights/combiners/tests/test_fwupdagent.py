...
from insights.combiners import fwupdagent
from insights.combiners.fwupdagent import FwupdagentDevicesHW, FwupdagentSecurityHW
...
def test_devices():
    ...
    result = FwupdagentDevicesHW(devices, virt_what)
    assert result
    ...
    with pytest.raises(SkipComponent):
        FwupdagentDevicesHW(devices, virt_what)


def test_security():
    security = FwupdagentSecurity(context_wrap(SECURITY))
    virt_what = VirtWhat(None, VWP(context_wrap("")))
    result = FwupdagentSecurityHW(security, virt_what)
    assert result
    assert len(result.security) == 2

    devices = FwupdagentSecurity(context_wrap(DEVICES))
    virt_what = VirtWhat(None, VWP(context_wrap("kvm")))
    with pytest.raises(SkipComponent):
        FwupdagentSecurityHW(devices, virt_what)
...
def test_doc_examples():
    ...
    virt_what = VirtWhat(None, VWP(context_wrap("")))
    env = {
        "devices": FwupdagentDevicesHW(devices, virt_what),
        "security": FwupdagentSecurityHW(security, virt_what),
    }
