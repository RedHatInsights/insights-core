...
@combiner(requires=[IpLinkInfo, Netstat_I])
class NetworkStats(object):
    ...
    def __init__(self, local, shared):
        if Netstat_I in shared:
            netstat = shared[Netstat_I]
            self.data = netstat.data
            ldata = deepcopy(netstat.data)
            self.group_by_iface = self._group_by_iface(ldata)
        elif IpLinkInfo in shared:
            nstat = shared.get(IpLinkInfo)
            self.group_by_iface, self.data = self._organize_for_netstat(nstat.data)
