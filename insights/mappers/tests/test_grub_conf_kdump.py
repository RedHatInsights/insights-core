import unittest
from insights.mappers import ParseException
from insights.mappers.grub_conf import GrubConfig
from insights.mappers.grub1_conf import Grub1Config
from insights.mappers.grub2_conf import Grub2Config
...
class Grub2ModuleTest(unittest.TestCase):

    def test_mod_internal(self):
        config = Grub2Config(context_wrap(MODULE_TEST))
        assert config
        assert type(config.kernels_initrds) == dict
        # Why is this two separate lists and not a list of dicts?
        assert 'grub_initrds' in config.kernels_initrds
        assert config.kernels_initrds['grub_initrds'] == [
            'initramfs-3.10.0-327.36.3.el7.x86_64.img',
        ]
        assert 'grub_kernels' in config.kernels_initrds
        assert config.kernels_initrds['grub_kernels'] == [
...
class TestGrub1ConfKdump(unittest.TestCase):

    def test_kdump_iommu_enabled(self):
        assert Grub1Config(context_wrap(IOMMU_OFF)).is_kdump_iommu_enabled is None
        assert Grub1Config(context_wrap(IOMMU_MISSING)).is_kdump_iommu_enabled is None
...
class TestGrubConfig(unittest.TestCase):
    ...
    def test_grub_config(self):
        config = GrubConfig(context_wrap(GOOD_OFFSET_1))
        assert config
        ...
        assert config['title'][0][0] == ('title_name', 'Red Hat Enterprise Linux Server (2.6.32-431.17.1.el6.x86_64)')
        assert config['title'][0][1] == ('kernel', '/vmlinuz-2.6.32-431.17.1.el6.x86_64 crashkernel=128M rhgb quiet')
        assert len(config['title'][1]) == 2
        assert config['title'][1][0] == ('title_name', 'Red Hat Enterprise Linux Server (2.6.32-431.11.2.el6.x86_64)')
        assert config['title'][1][1] == ('kernel', '/vmlinuz-2.6.32-431.11.2.el6.x86_64 crashkernel=128M rhgb quiet')

        # GRUB 1 config has no menu entries
        assert config['menuentry'] == []

        assert config.crash_kernel_offset is None
        assert config.is_kdump_iommu_enabled is None
        assert type(config.kernels_initrds) == dict
        # Why is this two separate lists and not a list of dicts?
...
class TestGrub1Config(unittest.TestCase):
    """
    #boot=/dev/sda
    default 0
    timeout=0
...
class TestGrub2Config(unittest.TestCase):
    ...
    def test_grub2_config(self):
        ...
        assert config['menuentry'][0][0] == ('menuentry_name', "'Red Hat Enterprise Linux Workstation (3.10.0-327.36.3.el7.x86_64) 7.2 (Maipo)' --class red --class gnu-linux --class gnu --class os --unrestricted $menuentry_id_option 'gnulinux-3.10.0-123.13.2.el7.x86_64-advanced-fbff9f50-62c3-484e-bca5-d53f672cda7c'")
...
#        assert config['menuentry'][0][-2] == ('linux16', '/vmlinuz-3.10.0-327.36.3.el7.x86_64 root=/dev/RHEL7CSB/Root ro rd.lvm.lv=RHEL7CSB/Root rd.luks.uuid=luks-96c66446-77fd-4431-9508-f6912bd84194 crashkernel=auto  vconsole.keymap=us rd.lvm.lv=RHEL7CSB/Swap vconsole.font=latarcyrheb-sun16 rhgb quiet LANG=en_GB.utf8')
        assert config['menuentry'][0][-1] == ('initrd16', '/initramfs-3.10.0-327.36.3.el7.x86_64.img')
...
class TestGrub1ConfigException(unittest.TestCase):

    def test_grub1_config_Exception(self):
        with pytest.raises(ParseException):
            Grub1Config(context_wrap(GRUB2_CONFIG))


class TestGrub2ConfigException(unittest.TestCase):

    def test_grub2_config_Exception(self):
        with pytest.raises(ParseException):
            Grub2Config(context_wrap(GOOD_OFFSET_1))
